...
export function ObservationForm({ siteWalkId, observation, onSaved }: { siteWalkId: string; observation?: Observation; onSaved?: (observation: Observation) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(null);
    try {
      const input = { siteWalkId, title, description, summary: title, detail: description, observationType, source, confidence, status, stationOrLine: optional(stationOrLine), processId: optional(processId), areaId: optional(areaId), observedAt: new Date().toISOString(), recordedByUserId: dataset?.users[0]?.id ?? '', origin: source === 'Reported by client' ? 'client' as const : 'consultant' as const, assurance: source === 'Directly observed' ? 'observed-fact' as const : 'interpreted' as const, visibility: 'internal' as const, aiStatus: 'not-applicable' as const, evidenceIds: observation?.evidenceIds ?? [], internalNotes: optional(internalNotes) };
...
export function EvidenceForm({ siteWalkId, observationId, onSaved }: { siteWalkId: string; observationId?: string; onSaved?: (evidence: Evidence) => void }) {
  const { dataset, createEvidence } = useFabricData();
  const [error, setError] = useState<string | null>(null);
